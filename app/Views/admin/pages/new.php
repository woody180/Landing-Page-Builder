...
<section class="uk-section">
    <div class="uk-container min-height uk-margin-large-top">
        ...
        <form action="<?= baseUrl("page") ?>" method="POST" class="uk-child-width-1-1" uk-grid>
            <?= csrf_field() ?>
            <div>
                <label for="" class="uk-form-label">Title</label>
                <input type="text" name="title" class="uk-input uk-border-rounded" required>
            </div>

            <div>
                <label for="" class="uk-form-label">Select layout for page</label>
                <select class="uk-border-rounded uk-select" name="layout">
                    <?php foreach (initModel('layout')->layoutList() as $layout): ?>
            ...
            <div>
                ...
                <textarea name="description" class="uk-textarea uk-height-small uk-border-rounded" required></textarea>
            </div>
            
            <div>
                <button class="uk-width-1-1 uk-button uk-button-primary">Add new page</button>
            </div>
        </form>
